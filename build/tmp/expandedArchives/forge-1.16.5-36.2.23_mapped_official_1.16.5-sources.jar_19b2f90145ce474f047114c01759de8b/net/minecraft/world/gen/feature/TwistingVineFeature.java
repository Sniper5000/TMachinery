...
public class TwistingVineFeature extends Feature<NoFeatureConfig> {
   ...
   public static boolean place(IWorld p_236423_0_, Random p_236423_1_, BlockPos p_236423_2_, int p_236423_3_, int p_236423_4_, int p_236423_5_) {
      if (isInvalidPlacementLocation(p_236423_0_, p_236423_2_)) {
         return false;
      } else {
         placeTwistingVines(p_236423_0_, p_236423_1_, p_236423_2_, p_236423_3_, p_236423_4_, p_236423_5_);
         return true;
      }
   }
   ...
   private static void placeTwistingVines(IWorld p_236424_0_, Random p_236424_1_, BlockPos p_236424_2_, int p_236424_3_, int p_236424_4_, int p_236424_5_) {
      ...
      for(int i = 0; i < p_236424_3_ * p_236424_3_; ++i) {
         blockpos$mutable.set(p_236424_2_).move(MathHelper.nextInt(p_236424_1_, -p_236424_3_, p_236424_3_), MathHelper.nextInt(p_236424_1_, -p_236424_4_, p_236424_4_), MathHelper.nextInt(p_236424_1_, -p_236424_3_, p_236424_3_));
         if (findFirstAirBlockAboveGround(p_236424_0_, blockpos$mutable) && !isInvalidPlacementLocation(p_236424_0_, blockpos$mutable)) {
            ...
            int k = 17;
            int l = 25;
            placeWeepingVinesColumn(p_236424_0_, p_236424_1_, blockpos$mutable, j, 17, 25);
         }
      }
   ...
   public static void placeWeepingVinesColumn(IWorld p_236422_0_, Random p_236422_1_, BlockPos.Mutable p_236422_2_, int p_236422_3_, int p_236422_4_, int p_236422_5_) {
      for(int i = 1; i <= p_236422_3_; ++i) {
         if (p_236422_0_.isEmptyBlock(p_236422_2_)) {
            if (i == p_236422_3_ || !p_236422_0_.isEmptyBlock(p_236422_2_.above())) {
               p_236422_0_.setBlock(p_236422_2_, Blocks.TWISTING_VINES.defaultBlockState().setValue(AbstractTopPlantBlock.AGE, Integer.valueOf(MathHelper.nextInt(p_236422_1_, p_236422_4_, p_236422_5_))), 2);
               break;
            }

            p_236422_0_.setBlock(p_236422_2_, Blocks.TWISTING_VINES_PLANT.defaultBlockState(), 2);
         }
   ...
   private static boolean isInvalidPlacementLocation(IWorld p_236421_0_, BlockPos p_236421_1_) {
      if (!p_236421_0_.isEmptyBlock(p_236421_1_)) {
         return true;
      } else {
         BlockState blockstate = p_236421_0_.getBlockState(p_236421_1_.below());
