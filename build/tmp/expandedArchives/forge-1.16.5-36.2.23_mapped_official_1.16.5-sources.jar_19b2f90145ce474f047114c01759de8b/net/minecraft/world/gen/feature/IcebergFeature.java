...
public class IcebergFeature extends Feature<BlockStateFeatureConfig> {
   ...
   private void generateCutOut(Random p_205184_1_, IWorld p_205184_2_, int p_205184_3_, int p_205184_4_, BlockPos p_205184_5_, boolean p_205184_6_, int p_205184_7_, double p_205184_8_, int p_205184_10_) {
      ...
      for(int i1 = 0; i1 < p_205184_4_ - 3; ++i1) {
         int j1 = this.heightDependentRadiusRound(p_205184_1_, i1, p_205184_4_, p_205184_3_);
         this.carve(j1, i1, p_205184_5_, p_205184_2_, false, d0, blockpos, p_205184_7_, p_205184_10_);
      }

      for(int k1 = -1; k1 > -p_205184_4_ + p_205184_1_.nextInt(5); --k1) {
         int l1 = this.heightDependentRadiusSteep(p_205184_1_, -k1, p_205184_4_, p_205184_3_);
         this.carve(l1, k1, p_205184_5_, p_205184_2_, true, d0, blockpos, p_205184_7_, p_205184_10_);
      }
   ...
   private void carve(int p_205174_1_, int p_205174_2_, BlockPos p_205174_3_, IWorld p_205174_4_, boolean p_205174_5_, double p_205174_6_, BlockPos p_205174_8_, int p_205174_9_, int p_205174_10_) {
      ...
      for(int k = -i; k < i; ++k) {
         for(int l = -i; l < i; ++l) {
            ...
            if (d0 < 0.0D) {
               BlockPos blockpos = p_205174_3_.offset(k, p_205174_2_, l);
               Block block = p_205174_4_.getBlockState(blockpos).getBlock();
               if (this.isIcebergBlock(block) || block == Blocks.SNOW_BLOCK) {
                  if (p_205174_5_) {
                     this.setBlock(p_205174_4_, blockpos, Blocks.WATER.defaultBlockState());
                  } else {
                     this.setBlock(p_205174_4_, blockpos, Blocks.AIR.defaultBlockState());
   ...
   private void removeFloatingSnowLayer(IWorld p_205185_1_, BlockPos p_205185_2_) {
      if (p_205185_1_.getBlockState(p_205185_2_.above()).is(Blocks.SNOW)) {
         this.setBlock(p_205185_1_, p_205185_2_.above(), Blocks.AIR.defaultBlockState());
      }
   ...
   private void generateIcebergBlock(IWorld p_205181_1_, Random p_205181_2_, BlockPos p_205181_3_, int p_205181_4_, int p_205181_5_, int p_205181_6_, int p_205181_7_, int p_205181_8_, int p_205181_9_, boolean p_205181_10_, int p_205181_11_, double p_205181_12_, boolean p_205181_14_, BlockState p_205181_15_) {
      double d0 = p_205181_10_ ? this.signedDistanceEllipse(p_205181_5_, p_205181_7_, BlockPos.ZERO, p_205181_9_, this.getEllipseC(p_205181_6_, p_205181_4_, p_205181_11_), p_205181_12_) : this.signedDistanceCircle(p_205181_5_, p_205181_7_, BlockPos.ZERO, p_205181_8_, p_205181_2_);
      if (d0 < 0.0D) {
         BlockPos blockpos = p_205181_3_.offset(p_205181_5_, p_205181_6_, p_205181_7_);
         double d1 = p_205181_10_ ? -0.5D : (double)(-6 - p_205181_2_.nextInt(3));
         if (d0 > d1 && p_205181_2_.nextDouble() > 0.9D) {
            return;
         }

         this.setIcebergBlock(blockpos, p_205181_1_, p_205181_2_, p_205181_4_ - p_205181_6_, p_205181_4_, p_205181_10_, p_205181_14_, p_205181_15_);
      }
   ...
   private void setIcebergBlock(BlockPos p_205175_1_, IWorld p_205175_2_, Random p_205175_3_, int p_205175_4_, int p_205175_5_, boolean p_205175_6_, boolean p_205175_7_, BlockState p_205175_8_) {
      BlockState blockstate = p_205175_2_.getBlockState(p_205175_1_);
      if (blockstate.getMaterial() == Material.AIR || blockstate.is(Blocks.SNOW_BLOCK) || blockstate.is(Blocks.ICE) || blockstate.is(Blocks.WATER)) {
         ...
         if (p_205175_7_ && !blockstate.is(Blocks.WATER) && (double)p_205175_4_ <= (double)p_205175_3_.nextInt(Math.max(1, p_205175_5_ / i)) + (double)p_205175_5_ * 0.6D && flag) {
            this.setBlock(p_205175_2_, p_205175_1_, Blocks.SNOW_BLOCK.defaultBlockState());
         } else {
            this.setBlock(p_205175_2_, p_205175_1_, p_205175_8_);
         }
      }
   ...
   private boolean belowIsAir(IBlockReader p_205182_1_, BlockPos p_205182_2_) {
      return p_205182_1_.getBlockState(p_205182_2_.below()).getMaterial() == Material.AIR;
   }
      ...
      for(int j = -i; j <= i; ++j) {
         for(int k = -i; k <= i; ++k) {
            for(int l = 0; l <= p_205186_4_; ++l) {
               BlockPos blockpos = p_205186_2_.offset(j, l, k);
               Block block = p_205186_1_.getBlockState(blockpos).getBlock();
               if (this.isIcebergBlock(block) || block == Blocks.SNOW) {
                  if (this.belowIsAir(p_205186_1_, blockpos)) {
                     this.setBlock(p_205186_1_, blockpos, Blocks.AIR.defaultBlockState());
                     this.setBlock(p_205186_1_, blockpos.above(), Blocks.AIR.defaultBlockState());
