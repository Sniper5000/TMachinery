...
public class ObjectiveArgument implements ArgumentType<String> {
   ...
   private static final DynamicCommandExceptionType ERROR_OBJECTIVE_NOT_FOUND = new DynamicCommandExceptionType((p_208671_0_) -> {
      return new TranslationTextComponent("arguments.objective.notFound", p_208671_0_);
   });
   private static final DynamicCommandExceptionType ERROR_OBJECTIVE_READ_ONLY = new DynamicCommandExceptionType((p_208669_0_) -> {
      return new TranslationTextComponent("arguments.objective.readonly", p_208669_0_);
   });
   public static final DynamicCommandExceptionType ERROR_OBJECTIVE_NAME_TOO_LONG = new DynamicCommandExceptionType((p_208670_0_) -> {
   ...
   public static ScoreObjective getObjective(CommandContext<CommandSource> p_197158_0_, String p_197158_1_) throws CommandSyntaxException {
      String s = p_197158_0_.getArgument(p_197158_1_, String.class);
      Scoreboard scoreboard = p_197158_0_.getSource().getServer().getScoreboard();
   ...
   public static ScoreObjective getWritableObjective(CommandContext<CommandSource> p_197156_0_, String p_197156_1_) throws CommandSyntaxException {
      ScoreObjective scoreobjective = getObjective(p_197156_0_, p_197156_1_);
      if (scoreobjective.getCriteria().isReadOnly()) {
         throw ERROR_OBJECTIVE_READ_ONLY.create(scoreobjective.getName());
      } else {
         return scoreobjective;
      }
   }
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> p_listSuggestions_1_, SuggestionsBuilder p_listSuggestions_2_) {
      if (p_listSuggestions_1_.getSource() instanceof CommandSource) {
         return ISuggestionProvider.suggest(((CommandSource)p_listSuggestions_1_.getSource()).getServer().getScoreboard().getObjectiveNames(), p_listSuggestions_2_);
         ...
         return isuggestionprovider.customSuggestion((CommandContext<ISuggestionProvider>)p_listSuggestions_1_, p_listSuggestions_2_);
      } else {
