...
public class WallProperty extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      return this.fixTypeEverywhereTyped("WallPropertyFix", this.getInputSchema().getType(TypeReferences.BLOCK_STATE), (p_233416_0_) -> {
         return p_233416_0_.update(DSL.remainderFinder(), WallProperty::upgradeBlockStateTag);
      });
   }
   ...
   private static <T> Dynamic<T> fixWallProperty(Dynamic<T> p_233418_0_, String p_233418_1_) {
      return p_233418_0_.update(p_233418_1_, (p_233421_0_) -> {
         return DataFixUtils.orElse(p_233421_0_.asString().result().map(WallProperty::mapProperty).map(p_233421_0_::createString), p_233421_0_);
      });
   }

   private static <T> Dynamic<T> upgradeBlockStateTag(Dynamic<T> p_233417_0_) {
      boolean flag = p_233417_0_.get("Name").asString().result().filter(WALL_BLOCKS::contains).isPresent();
      return !flag ? p_233417_0_ : p_233417_0_.update("Properties", (p_233420_0_) -> {
         Dynamic<?> dynamic = fixWallProperty(p_233420_0_, "east");
